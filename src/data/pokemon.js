// Pokemon name data for display
// This is a simplified list - in production you'd fetch from PokeAPI
...
// Get Pokemon name by national dex number
export function getPokemonName(dexNumber) {
  return POKEMON_NAMES[dexNumber] || `Pokemon #${dexNumber}`;
}

// Get sprite URL from Pokemon HOME (3D models)
// Using the official Pokemon HOME sprites which are consistent 3D renders
export function getPokemonSprite(dexNumber, shiny = false) {
  // Using PokeAPI's official-artwork or sprites
  // For 3D models we'll use the HOME sprites
  const paddedNumber = String(dexNumber).padStart(3, '0');
  
  if (shiny) {
    // Shiny sprites from PokeAPI
    return `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/home/shiny/${dexNumber}.png`;
  }
  
  // Regular sprites from Pokemon HOME
  return `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/home/${dexNumber}.png`;
}

// Get all Pokemon data for a specific game
export function getAllPokemonForGame(gameId, pokemonNumbers) {
  return pokemonNumbers.map(num => ({
    dexNumber: num,
    name: getPokemonName(num),
    sprite: getPokemonSprite(num, true), // Shiny variant as requested
  }));
}
